...
const register = async (req, res) => {
  ...
  if (!email || !first_name, !last_name, !password || !username ) return res.status(401).json({ message: "All fields must be filled", success: false })
  ...
  await _userinfo.save().then(resp => {
    ...
    if (err.keyPattern) return res.status(500).json({ success: false, payload: null, error: "Email and username must be unique" })

   return res.status(500).json({ success: false, payload: null, error: "unexpected error" })
  })
}
...
const sendOtp = async (req, res) => {
  ...
  try {
    ...
    sendMail({ email, otp: otp, subject: "Bank-app email verification otp" }).then(resp => res.send( res.json({ message: resp.msg, success: true })))
...
module.exports = { register, updateProfile, login, sendOtp, verify_otp, fileUpload, profile }
